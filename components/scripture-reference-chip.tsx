...
export function ScriptureReferenceChip({
  ...
  const href = getLocalHebrewsReferenceHref(reference);
...
function getLocalHebrewsReferenceHref(reference: string) {
  const match = reference.trim().match(/^Hebrews\s+(\d+):(\d+)(?:[-–—]\d+)?$/iu);
  if (!match) return null;

  return `/hebrews/${match[1]}/#v${match[2]}`;
}
